...
class StatisticRepositoryImpl implements StatisticRepository {
  ...
  Future<StatisticOverviewData?> getOverview({
    ...
    try {
      ...
      final results = await Future.wait<Object?>([
        ...
        _fetchIncomeTransactionsForRange(
          userId: userId,
          start: currentRange.start,
          end: currentRange.end,
        ),
      ...
      final currentIncomeTransactions = results[2]! as List<TransactionModel>;
      final currentSummary = results[3] as Map<String, dynamic>?;
      final previousSummary = results[4] as Map<String, dynamic>?;
  ...
  Future<List<TransactionModel>> _fetchIncomeTransactionsForRange({
    required String userId,
    required DateTime start,
    required DateTime end,
  }) async {
...
StatisticDateRange statisticGetPeriodRange({
  ...
  if (selectedTabIndex == 2) {
    return StatisticDateRange(
      ...
      end: DateTime(anchorDate.year, 12, 31, 23, 59, 59, 999),
    );
  }
...
StatisticDateRange statisticGetPreviousPeriodRange({
  ...
  if (selectedTabIndex == 2) {
    return StatisticDateRange(
      ...
      end: DateTime(anchorDate.year - 1, 12, 31, 23, 59, 59, 999),
    );
  }
...
StatisticPeriodOverview buildStatisticPeriodOverview({
  ...
  List<TransactionModel> incomeTransactions = const [],
  ...
  final totalExpenseMinor = _readSummaryTotalExpenseMinor(summaryData) ??
      _sumTransactionAmountMinor(transactions);
  ...
  final categories = _buildCategoryData(
    ...
    totalAmountMinor: totalExpenseMinor,
  );

  final incomeCategories = _buildIncomeCategories(incomeTransactions);
  final totalIncomeMinor = _sumTransactionAmountMinor(incomeTransactions);
...
/// Aggregates income transactions into top-category data, mirroring the expense
/// aggregation. Income has no period-over-period trend, so the previous totals
/// are intentionally empty and every entry resolves to [StatisticTrendDirection.none].
List<StatisticCategoryData> _buildIncomeCategories(
  ...
  final totalIncomeMinor = _sumTransactionAmountMinor(incomeTransactions);
  return _buildCategoryData(
    currentCategoryTotals: _buildCategoryMap(incomeTransactions),
    previousCategoryTotals: const {},
    transactionCountByCategory:
        _countTransactionsByCategory(incomeTransactions),
    categoryLabelByKey: _readCategoryLabels(incomeTransactions),
    iconByCategory: _readCategoryIcons(incomeTransactions),
    totalAmountMinor: totalIncomeMinor,
  );
}

int _sumTransactionAmountMinor(List<TransactionModel> transactions) {
...
Map<String, String> _readCategoryLabels(List<TransactionModel> transactions) {
  final localMap = {
    for (final c in AppConstants.defaultTransactionCategories) c.id: c.label,
  };
  final labels = <String, String>{};
  for (final transaction in transactions) {
    final key = transaction.categoryKey;
    labels[key] = localMap[key] ?? transaction.categoryLabel;
  }
  return labels;
}
...
Map<String, String?> _readCategoryIcons(List<TransactionModel> transactions) {
  final localMap = {
    for (final c in AppConstants.defaultTransactionCategories) c.id: c.icon,
  };
  final icons = <String, String?>{};
  for (final transaction in transactions) {
    final key = transaction.categoryKey;
    icons[key] = localMap[key] ?? transaction.categoryIcon;
  }
  return icons;
}
...
List<StatisticCategoryData> _buildCategoryData({
  ...
  final results = allCategoryKeys.map((categoryKey) {
    ...
    final shareRatio = totalAmountMinor > 0
        ? currentAmount / totalAmountMinor
        : 0.0;
...
int? _readSummaryTotalExpenseMinor(Map<String, dynamic>? summaryData) {
  ...
  return (summaryData['total_expense'] as num?)?.toInt();
}
